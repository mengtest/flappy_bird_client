...
atlasTexture = cc.Director:getInstance():getTextureCache():addImage("atlas.png")

--获得atlas sprite
function createAtlasSprite(name)
    if atlas[name] == nil then return nil end
    local pos = atlas[name]
    local rect = cc.rect(pos.x, pos.y, pos.width, pos.height)
...
--获得atlas frame
function createAtlasFrame(name)
    if atlas[name] == nil then return nil end
    local pos = atlas[name]
    local rect = cc.rect(pos.x, pos.y, pos.width, pos.height)
    local frame = cc.SpriteFrame:createWithTexture(atlasTexture, rect)
